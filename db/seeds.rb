...
new_users_needed.times do
   user = { user_name: Faker::Name.name, email: Faker::Internet.email, password: Faker::Coffee.blend_name }
   user = User.new(user)
   user.save
end
...
new_kudos_needed.times do
   kudo = { user_name: Faker::Name.name, subject: Faker::Book.title, description: Faker::Coffee.blend_name, applause: rand(1..20) }
   kudo = Kudo.new(user)
   kudo.save
end
